...
async def pause(_, message: Message):
    if (
        ...
        await message.reply_text(f"**{BN} :** Play Something. Nothing Is Playing.")
    else:
...
async def resume(_, message: Message):
    if (
        ...
        await message.reply_text(f"**{BN} :** Try Pausing. Nothing is paused.")
    else:
...
async def stop(_, message: Message):
    if message.chat.id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text(f"**{BN} :** Try Streaming. Nothing is streaming.")
    else:
        try:
...
async def skip(_, message: Message):
    if message.chat.id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text(f"**{BN} :** Try Playing. Nothing Is Playing To Skip.")
    else:
...
async def admincache(_, message: Message):
    ...
    await message.reply_text(f"**{BN} :** ❇ refreshed!")
